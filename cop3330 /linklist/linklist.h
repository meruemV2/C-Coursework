...
// *     sex, and whether the person believes cheesecake is pie or cake.      *
// *                                                                         *
// * Author: Dr. Gaitros                                                     *
...
public:
    ...
    bool Find(const int idnumber);   	// Looks for a record with the id, returns true if
    ...
    bool Delete();			// Deletes current record. Must use Find() to locate
    //   the record.
    void Print(); 			// Prints the report..see example.
    void List(); 			// Lists current record to standard output. See Example.
    //   Issues an error message of current is null.
    int CountPie();			// Returns number of records that thought Cheese Cake is
    //   is Pie.
    int CountCake();           	// Returns the number of records that thought Cheese Cake
    //   is Cake.
private:
